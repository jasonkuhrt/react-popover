...
let Popover = createClass({
  ...
  render() {
    log('React render')
    //this._layerRender()
    //let deepChildren = this.props.children.props.children
    //let cs = (deepChildren ? Array.isArray(deepChildren) ? deepChildren : [deepChildren] : []).concat([popover])
    //console.log(cs)
    //let p = React.cloneElement.apply(null, [this.props.children, { ref: 'foo' }].concat(cs[1]))
