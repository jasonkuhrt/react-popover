...
const Popover = createClass({
  ...
  propTypes: {
    ...
    animationHandlers: T.shape({
      exit: T.func.isRequired,
      exitStop: T.func.isRequired,
      enter: T.func.isRequired,
      enterStop: T.func.isRequired,
    }),
  },
  getInitialState () {
    return {
  ...
  getDefaultProps () {
    return {
      ...
      animationHandlers: {
        exitStop (popover) {
          clearTimeout(popover.exitingAnimationTimer1)
          clearTimeout(popover.exitingAnimationTimer2)
        },
        exit (popover, completionCallback) {
          popover.exitingAnimationTimer2 = setTimeout(() => {
            setTimeout(() => {
              popover.containerEl.style.transform = `${flowToPopoverTranslations[popover.zone.flow]}(${popover.zone.order * 50}px)`
              popover.containerEl.style.opacity = `0`
            }, 0)
          }, 0)

          popover.exitingAnimationTimer1 = setTimeout(completionCallback, popover.props.enterExitTransitionDurationMs)
        },
        enterStop (popover) {
          clearTimeout(popover.enteringAnimationTimer1)
          clearTimeout(popover.enteringAnimationTimer2)
        },
        enter (popover) {
          /* Prepare `entering` style so that we can then animate it toward `entered`. */

          popover.containerEl.style.transform = `${flowToPopoverTranslations[popover.zone.flow]}(${popover.zone.order * 50}px)`
          popover.containerEl.style[jsprefix(`Transform`)] = popover.containerEl.style.transform
          popover.containerEl.style.opacity = `0`

          /* After initial layout apply transition animations. */

          popover.enteringAnimationTimer1 = setTimeout(() => {
            popover.tipEl.style.transition = `transform 150ms ease-in`
            popover.tipEl.style[jsprefix(`Transition`)] = `${cssprefix(`transform`)} 150ms ease-in`
            popover.containerEl.style.transitionProperty = `top, left, opacity, transform`
            popover.containerEl.style.transitionDuration = `500ms`
            popover.containerEl.style.transitionTimingFunction = `cubic-bezier(0.230, 1.000, 0.320, 1.000)`
            popover.enteringAnimationTimer2 = setTimeout(() => {
              popover.containerEl.style.opacity = `1`
              popover.containerEl.style.transform = `translateY(0)`
              popover.containerEl.style[jsprefix(`Transform`)] = popover.containerEl.style.transform
            }, 0)
          }, 0)
        }
  ...
  animateExitStop () {
    this.props.animationHandlers.exitStop(this)
    this.setState({ exiting: false })
  },
  animateExit () {
    this.setState({ exiting: true })
    this.props.animationHandlers.exit(this, () => {

      // wait for next tick before unrendering
      setTimeout(() => {
  ...
  animateEnterStop () {
    this.props.animationHandlers.enterStop(this)
  },
  animateEnter () {
    this.props.animationHandlers.enter(this)
  },
  trackPopover () {
