  ...
  getDefaultProps () {
    return {
      ...
      animationHandlers: {
        ...
        enterStop: function noOperation () {},
        enter (popover) {
          /* Prepare `entering` style so that we can then animate it toward `entered`. */

          popover.containerEl.style.transform = `${flowToPopoverTranslations[this.zone.flow]}(${this.zone.order * 50}px)`
          ...
          popover.containerEl.offsetHeight
