...
export default createClass({
  ...
  doResolve() {
    ...
    let frameBounds = calcBounds(this.frameEl)
    let zone = resolveZoneChoice(this.p2, frameBounds)
    ...
    if (p[axis.cross.start] < frameBounds[axis.cross.start]) {
      p[axis.cross.start] = frameBounds[axis.cross.start]
    } else if (p[axis.cross.end] > frameBounds[axis.cross.end]) {
      p[axis.cross.start] = p[axis.cross.start] - (p[axis.cross.end] - frameBounds[axis.cross.end])
    }

    log('layoutOrientation is %s', orientation)
    log('popover cross-start/end is %d-%d', p[axis.cross.start], p[axis.cross.end])
    log('frame cross-start/end is %d-%d', frameBounds[axis.cross.start], frameBounds[axis.cross.end])
