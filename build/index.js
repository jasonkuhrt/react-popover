...
let _extends = Object.assign || function (target) { for (let i = 1; i < arguments.length; i++) { const source = arguments[i]; for (const key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key] } } } return target }

let _createClass = function () { function defineProperties (target, props) { for (let i = 0; i < props.length; i++) { const descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor) } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor } }()

let _cssVendor = require("css-vendor")

let cssVendor = _interopRequireWildcard(_cssVendor)

let _debug = require("debug")

let _debug2 = _interopRequireDefault(_debug)
...
let _layout = require("./layout")

let _layout2 = _interopRequireDefault(_layout)

let _onResize = require("./on-resize")

let _onResize2 = _interopRequireDefault(_onResize)

let _platform = require("./platform")

let _platform2 = _interopRequireDefault(_platform)

let _tip = require("./tip")

let _tip2 = _interopRequireDefault(_tip)
...
const log = (0, _debug2.default)("react-popover")

let supportedCSSValue = _utils2.default.clientOnly(cssVendor.supportedValue)

let jsprefix = function jsprefix (x) {
  return String(cssVendor.prefix.js) + x
}

let cssprefix = function cssprefix (x) {
  return String(cssVendor.prefix.css) + x
}

let cssvalue = function cssvalue (prop, value) {
...
let Popover = function (_React$Component) {
  ...
  function Popover (props) {
    ...
    let _this = _possibleConstructorReturn(this, (Popover.__proto__ || Object.getPrototypeOf(Popover)).call(this, props))
    ...
    _this.checkForOuterAction = function (event) {
      const isOuterAction = !_this.containerEl.contains(event.target) && !_this.targetEl.contains(event.target)
    ...
    _this.getContainerNodeRef = function (containerEl) {
      Object.assign(_this, { containerEl })
    }
  ...
  _createClass(Popover, [{
    ...
    value: function resolvePopoverLayout () {
      ...
      const zone = _layout2.default.pickZone(pickerSettings, this.frameBounds, this.targetBounds, this.size)
      if (this.zone) {
        this.size[this.zone.flow === "row" ? "h" : "w"] -= this.props.tipSize
      }
      ...
      let axis = _layout2.default.axes[zone.flow]
      log("axes", axis)

      let dockingEdgeBufferLength = Math.round(getComputedStyle(this.bodyEl).borderRadius.slice(0, -2)) || 0
      let scrollSize = _layout2.default.El.calcScrollSize(this.frameEl)
      ...
      let pos = _layout2.default.calcRelPos(zone, tb, this.size)
    ...
    value: function measureTargetBounds () {
      const newTargetBounds = _layout2.default.El.calcBounds(this.targetEl)
    ...
    value: function render () {
      ...
      let standing = this.state.standing
      ...
      let popover = this.state.exited ? null : _react2.default.createElement(
        "div",
