...
##### `onOuterMovement :: (Event) -> Void`
A callback function executed every time the user moves (`mouseenter` or `mouseover`) outside the DOM tree of both `Popover` and `Target`. A canonical use-case is to automatically close the Popover on any external user movement.
