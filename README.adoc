...
- `onOuterAction : Function` : A callback function executed very time the user does an action (`mousedown` or `touchstart`) that is outside the DOM tree of both `Popover` and `Target`. A canonical use-case is to automatically close the Popover on any external action.
